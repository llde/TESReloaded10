...
float3 getWaterNorm(float2 pos) {
	...
	float4 sampledResult = tex2D(NormalMap, pos.xy / (624 * waveWidth));
...
PS_OUTPUT main(VS_OUTPUT IN, float2 PixelPos : VPOS) {
	...
	//Reflection
	float3 eye = normalize(EyePos.xyz - IN.texcoord_1.xyz);
	float cos = dot(eye, normal.xyz);
	float4 reflection = texCUBE(ReflectionMap, eye - ((2 * cos) * normal.xyz));
