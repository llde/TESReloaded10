...
float3 getWaterNorm(float2 pos, float2 dispPos, float blend, inout float height) {
	...
	float3 displacementNormal = tex2D(DisplacementMap, dispPos.xy).rgb * 1.2 - 0.6;
...
PS_OUTPUT main(VS_OUTPUT IN, float2 PixelPos : VPOS){
	...
	float blend = saturate(4 * (BlendRadius.x - (2 * length(IN.texcoord_0.xy - 0.5))));
	
	float3 normal = getWaterNorm(surfPos, IN.texcoord_0.xy, blend, height);
	eyepos.z += height;
	...
	refract_color.rgb *= 1 + caustics * causticsAngle * TESR_SunColor.xyz;
